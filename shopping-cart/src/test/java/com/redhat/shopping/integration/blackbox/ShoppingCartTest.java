...
public class ShoppingCartTest {
	...
	private void addProductToTheCartWithIdAndRandomQuantity(int productId) {
		...
		given().contentType("application/json").body(productToAdd).put("/cart");
	}
	...
	public void removingNonExistingProductInCatalogReturns400() {
		given().pathParam("id", 9999).when().delete("/cart/products/{id}").then().statusCode(400);
	}

	@Test
	public void removingNonAddedProductToTheCartReturns404() {
		given().pathParam("id", 1).when().delete("/cart/products/{id}").then().statusCode(404);
	}
}
